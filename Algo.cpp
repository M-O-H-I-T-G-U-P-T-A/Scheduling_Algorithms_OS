...
int num_of_process=3, cnt, remain, time_quantum;
...
void get_process_info(int selected_algo){
	...
	for (int i = 0; i < num_of_process; i++){
		...
		else if (selected_algo == 2 || selected_algo==5){
			cout << "=> Arrival Time: "; 
			cin >> arrival_time[i];
  			cout << "=> Burst Time: "; 
  			cin >> burst_time[i];
...
int get_observation_time(int selected_algo){
	if (selected_algo < 3 || selected_algo==5 ){
		int sum=0;
		for(int i=0; i<num_of_process; i++){
			sum += burst_time[i];
		}
...
void shortest_job_first(int cycles){
    ...
    //copying burst_time array to remain_time array
    for (int i = 0; i < num_of_process; i++){
        remain_time[i] = burst_time[i];
    ...
    while (cnt != num_of_process){
        ...
        for (int i = 0; i < num_of_process; i++){
            if (arrival_time[i] <= time && remain_time[i] > 0){
                if (shortest_process == -1 || remain_time[i] < remain_time[shortest_process]){
                    shortest_process = i;
                }
            }
        }
...
void round_robin(int time){
	int i, current_time = 0, temp_process, flag = 0, cnt = 0; 
	...
	for(i = 0; temp_process != 0;) { 
		process_cycle[cnt] = current_time;
		cnt++;
    ...
    for (i=0,cnt=0; i<time; i++){
    	if (i == time - 1){
    		update_process_cycle[cnt] = process_cycle[i];
    	}
    ...
    update_process_cycle[cnt] = current_time; //adding last completion time    
